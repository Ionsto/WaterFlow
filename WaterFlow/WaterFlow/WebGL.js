...
(function (WebGL) {
    var WebGLGame = (function () {
        ...
        WebGLGame.prototype.InitRenderCanvas = function () {
            try  {
                this.GL = world.RenderCanvas.getContext("webgl") || world.RenderCanvas.getContext("experimental-webgl");
                this.GL.viewport(0, 0, world.RenderCanvas.width, world.RenderCanvas.height);
            } catch (e) {
                console.log(e);
            }
            if (!this.GL) {
                console.log("Could not initialise WebGL");
                world.CanRenderWebGL = false;
                world.WillRenderWebGL = false;
            }
        };
        ...
        WebGLGame.prototype.InitBuffers = function () {
            ...
            var HSize = (world.WorldSize - 1) / 2;
            for (var x = 0; x < world.WorldSize; ++x) {
                for (var y = 0; y < world.WorldSize; ++y) {
            ...
            for (var x = 0; x < world.WorldSize - 1; ++x) {
                for (var y = 0; y < world.WorldSize - 1; ++y) {
                    var pos = x + (y * world.WorldSize);
                    Indicies.push(pos, pos + 1, pos + 1 + world.WorldSize);
                    Indicies.push(pos, pos + world.WorldSize, pos + 1 + world.WorldSize);
                }
            }
