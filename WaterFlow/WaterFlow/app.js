...
(function (normal) {
    var Grid = (function () {
        ...
        Grid.prototype.AddValueAt = function (x, y, v) {
            if (x >= 0 && x < this.SizeX) {
                if (y >= 0 && y < this.SizeY) {
                    this.RawGrid[(x * this.SizeX) + y] += v;
                    if (this.RawGrid[(x * this.SizeX) + y] > this.MaxHeight) {
    ...
    var World = (function () {
        function World() {
            ...
            this.FlowPerTick = 0.5;
            this.SiltPerTickDirect = 0.1;
            this.SiltPerTickIndirect = 1;
            this.SiltPoint = 0;
            this.SiltPerVolume = 100;
            this.Inflow = 10;
            this.OutFlow = 10;
            ////
            this.WaterHeight = new Grid(this.WorldSize, this.WorldSize);
            this.WorldType = new Grid(this.WorldSize, this.WorldSize);
            ...
            for (var x = 0; x < this.WaterHeight.SizeX; ++x) {
                for (var y = 0; y < this.WaterHeight.SizeY; ++y) {
                    ...
                    for (var i = 0; i < SearchSpace.length; ++i) {
                        ...
                        if (x - offset[0] < 0 || y - offset[1] < 0) {
                            value = null;
                        }
                        if (x - offset[0] > this.WaterHeight.SizeX || y - offset[1] > this.WaterHeight.SizeY) {
                            value = null;
                        }
        ...
        World.prototype.DirectSilting = function (x, y) {
            //Direct of the floor
        ...
        World.prototype.IndirectSilting = function (x, y) {
            ...
            for (var i = 0; i < SearchSpace.length; ++i) {
                ...
                if (X > this.WaterHeight.SizeX || Y > this.WaterHeight.SizeY) {
                    continue;
                }
                if (this.GroundHeight.GetValueAt(X, Y) > this.GroundHeight.GetValueAt(x, y)) {
                    var SiltPercent = 1 - (this.SiltMap.GetValueAt(x, y) / SiltCapacity);
        ...
        World.prototype.Erosion = function () {
            for (var x = 0; x < this.GroundHeight.SizeX; ++x) {
                for (var y = 0; y < this.GroundHeight.SizeY; ++y) {
                    if (this.WaterHeight.GetValueAt(x, y) > 0) {
                        var SearchSpace = [[1, 0], [0, 1], [-1, 0], [0, -1]];
                        ...
                        for (var i = 0; i < SearchSpace.length; ++i) {
                            ...
                            if (x - offset[0] > this.GroundHeight.SizeX || y - offset[1] > this.GroundHeight.SizeY) {
                                value = null;
                            }
                            AjoiningCellHeights.push([x - offset[0], y - offset[1], value]);
                        }
